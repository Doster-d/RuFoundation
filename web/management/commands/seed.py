...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("-a", "--archive", required=False,
                            help="Seeds from specified archive")
        parser.add_argument("-s", "--site-slug", required=True,
                            help="Slug for site that items will be added to")
        parser.add_argument("-f", "--forum", required=False, action='store_true',
                            help="Seeds the forum (only applicable for Archive)")
