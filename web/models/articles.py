...
class Tag(SiteLimitedModel):
    class Meta:
        ...
        constraints = [models.UniqueConstraint(
            fields=['name'], name='%(app_label)s_%(class)s_unique')]
...
class Category(SiteLimitedModel):
    ...
    users_can_view = models.BooleanField(
        verbose_name='Пользователи могут просматривать статьи', null=False, default=True)
    users_can_create = models.BooleanField(
        verbose_name='Пользователи могут создавать статьи', null=False, default=True)
    users_can_edit = models.BooleanField(
        verbose_name='Пользователи могут редактировать статьи', null=False, default=True)
    users_can_rate = models.BooleanField(
        verbose_name='Пользователи могут голосовать за статьи', null=False, default=True)
    users_can_comment = models.BooleanField(
        verbose_name='Пользователи могут комментировать статьи', null=False, default=True)
    users_can_delete = models.BooleanField(
        verbose_name='Пользователи могут удалять статьи', null=False, default=False)
    ...
    def get_settings(self):
        category_settings = Settings.objects.filter(
            category=self).first() or Settings.get_default_settings()
        site_settings = Settings.objects.filter(
            site=self.site).first() or Settings.get_default_settings()
...
class Article(SiteLimitedModel):
    class Meta:
        ...
        constraints = [models.UniqueConstraint(
            fields=['category', 'name'], name='%(app_label)s_%(class)s_unique')]
        indexes = [models.Index(fields=['category', 'name']), models.Index(
            fields=['created_at']), models.Index(fields=['updated_at'])]
    ...
    tags = models.ManyToManyField(
        Tag, blank=True, related_name="articles", verbose_name="Тэги")
    ...
    media_name = models.TextField(
        verbose_name="Название папки с файлами в ФС-хранилище", unique=True, default=uuid4)

    def get_settings(self):
        try:
            category_as_object = Category.objects.get(
...
class ArticleVersion(SiteLimitedModel):
    ...
    article = models.ForeignKey(
        Article, on_delete=models.CASCADE, verbose_name="Статья", related_name='versions')
    ...
    rendered = models.TextField(
        blank=True, null=True, verbose_name="Рендер статьи")
    created_at = models.DateTimeField(
        auto_now_add=True, verbose_name="Время создания")
...
class ArticleLogEntry(SiteLimitedModel):
    class Meta:
        ...
        constraints = [models.UniqueConstraint(
            fields=['article', 'rev_number'], name='%(app_label)s_%(class)s_unique')]
...
class Vote(SiteLimitedModel):
    class Meta:
        ...
        constraints = [models.UniqueConstraint(
            fields=['article', 'user'], name='%(app_label)s_%(class)s_unique')]
        indexes = [models.Index(fields=['article'])]

    article = models.ForeignKey(
        Article, on_delete=models.CASCADE, verbose_name="Статья", related_name='votes')
    user = models.ForeignKey(settings.AUTH_USER_MODEL, null=True,
...
class ExternalLink(SiteLimitedModel):
    class Meta:
        ...
        constraints = [models.UniqueConstraint(
            fields=['link_from', 'link_to', 'link_type'], name='%(app_label)s_%(class)s_unique')]
    ...
    link_type = models.TextField(
        choices=Type.choices, verbose_name="Тип ссылки", null=False)
