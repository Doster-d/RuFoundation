...
class ForumCategory(SiteLimitedModel):
    ...
    order = models.IntegerField(
        verbose_name="Порядок сортировки", default=0, blank=True)
    section = models.ForeignKey(to=ForumSection, on_delete=models.DO_NOTHING,
                                verbose_name="Категория")  # to-do: review later
    is_for_comments = models.BooleanField(
        verbose_name="Отображать комментарии к статьям в этом разделе", default=False)
...
class ForumThread(SiteLimitedModel):
    class Meta:
        ...
        constraints = [
            ...
            models.CheckConstraint(
                ...
                check=Exact(
                    lhs=Func('article_id', 'category_id', function='num_nonnulls',
    ...
    category = models.ForeignKey(to=ForumCategory, on_delete=models.DO_NOTHING,
                                 null=True, verbose_name="Раздел (если это тема)")  # to-do: review later
    article = models.ForeignKey(to=Article, on_delete=models.CASCADE,
                                null=True, verbose_name="Статья (если это комментарии)")
    author = models.ForeignKey(to=settings.AUTH_USER_MODEL,
                               on_delete=models.SET_NULL, null=True, verbose_name="Автор")
...
class ForumPost(SiteLimitedModel):
    ...
    reply_to = models.ForeignKey(
        to='ForumPost', on_delete=models.SET_NULL, null=True, verbose_name="Ответ на комментарий")
    thread = models.ForeignKey(
        to=ForumThread, on_delete=models.CASCADE, verbose_name="Тема")
...
class ForumPostVersion(SiteLimitedModel):
    ...
    post = models.ForeignKey(
        to=ForumPost, on_delete=models.CASCADE, verbose_name="Сообщение")
    source = models.TextField(verbose_name="Текст сообщения")
    created_at = models.DateTimeField(
        auto_now_add=True, verbose_name="Время создания")
    author = models.ForeignKey(to=settings.AUTH_USER_MODEL,
                               on_delete=models.SET_NULL, null=True, verbose_name="Автор правки")
